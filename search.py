...
def depthFirstSearch(problem):
    ...
    This implementation displays the starting and ending time of the search
    and the path taken to reach the goal.
    """
    # Record the start time
    start_time = time.time()
    print("DFS search started at:", time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(start_time)))
    ...
    while not frontier.isEmpty():
        ...
        if problem.isGoalState(currState):
            ...
            print("DFS search ended at:", time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(end_time)))
            print(f"Total time taken: {end_time - start_time:.4f} seconds")
            print("Path to goal:", currPath)
            return currPath
    ...
    # If no solution is found, print that information and return an empty path
    end_time = time.time()  # Record the end time
    print("DFS search ended at:", time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(end_time)))
    print(f"Total time taken: {end_time - start_time:.4f} seconds")
